...
BOOL CPidInput::Lf_setExecuteMesAGNOUT() // AGN_OUT
{
	...
	/*if (m_pApp->Gf_gmesSendHost(HOST_AGN_OUT, m_nMesAutoRackNo, NULL, NULL) == TRUE)
	{
		sLog.Format(_T("<MES> AGN_OUT Send OK - RACK%d"), m_nMesAutoRackNo + 1);
		m_pApp->Gf_writeMLog(sLog);
		Lf_addMessage(sLog);
	...
	{
		sLog.Format(_T("<MES> AGN_OUT Send NG - RACK%d"), m_nMesAutoRackNo + 1);
		m_pApp->Gf_writeMLog(sLog);
	}*/
